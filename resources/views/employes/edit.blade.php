...
@section('title', 'Modifier un employé — ERP')
...
@section('content')
    ...
    <div class="mb-8">
        ...
        <h1 class="text-xl font-semibold text-slate-900">
            Modifier l'employé : {{ $employe->nom }} {{ $employe->prenom }}
        </h1>
    </div>
            ...
            <form action="{{ route('employes.update', $employe->id_employe) }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="prenom" name="prenom" value="{{ old('prenom', $employe->prenom) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="telephone" name="telephone"
                        value="{{ old('telephone', $employe->telephone) }}" required class="{{ $input }}">
                    @error('telephone')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="date_embauche" name="date_embauche"
                        value="{{ old('date_embauche', $employe->date_embauche) }}" required class="{{ $input }}">
                    @error('date_embauche')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    <div>
                        <label for="actif" class="{{ $label }}">Employé actif ?</label>
                        <select name="actif" id="actif" class="{{ $input }}">
                    ...
                    <div>
                        <label for="date_debauche" class="{{ $label }}">Date de débauche</label>
                        <input type="date" name="date_debauche" id="date_debauche"
                            value="{{ old('date_debauche', $employe->date_debauche) }}" class="{{ $input }}">
                        @error('date_debauche')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
